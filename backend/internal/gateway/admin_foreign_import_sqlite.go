...
func (s *Server) import9routerTokenSaver(ctx context.Context, data map[string]any, res *foreignImportResult) {
	...
	// Headroom: headroomEnabled + headroomCodeAware. The upstream headroomUrl
	// is intentionally ignored: this fork bundles the Headroom runtime and no
	// longer exposes an operator-configurable proxy URL (EndpointSettings has
	// no HeadroomURL field).
