...
export function ChainModelPicker({ value, providers, onChange, autoFocus = false }: {
  ...
  const catalog = useModelCatalog({ connectedOnly: true });
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [custom, setCustom] = useState(false);
  // Chain steps may only target connected providers: unconnected providers
  // (no usable account) would produce dead routes.
  const connectedProviders = useMemo(() => providers.filter((provider) => provider.connected === true), [providers]);
  const availableProviderIDs = useMemo(() => new Set(connectedProviders.filter(isLLMProvider).map((provider) => provider.id)), [connectedProviders]);
